...
async def allUserInfo(cal:CallbackQuery):
    await cal.answer()
    await cal.message.answer("Barcha Foydalanuvchilar",reply_markup=all_user_info_inline(userDb.getAllUsers()))
...
async def foydalanishQollanmasi(cal:CallbackQuery):
    await cal.answer()
    await cal.message.answer_video(video="BAACAgIAAxkBAAMnZjr8Hi9zKzGlmQN2J1Tg_bzS6swAAqpMAAIvAAHISaG6WQiFblYiNQQ",caption="Botdan foydalanish qolanmasi!")
    
# async def getVideo(mes:Message):
#     await mes.answer(mes.video.file_id)