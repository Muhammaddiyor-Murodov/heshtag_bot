...
def asosy_menu(user_id,adminlarDb_data):
    lis=[
        ...
        [
            # qollanma
            InlineKeyboardButton(text="📒 Foydalanish Qollanmasi",callback_data='foydalanish_qollanmasi')
        ]
    ]
...
def all_user_info_inline(userDb_data):
    lis=[[InlineKeyboardButton(text=str(user[1])+f"  - imkoniyati: {user[-1]} ta",url=f"tg://user?id={user[0]}")] for user in userDb_data]
    return InlineKeyboardMarkup(
        inline_keyboard=lis
    )